...
export { type JWKJson, Key } from './jwk-key';
export { KeyStore } from './jwk-key-store';
 ...
 * The second argument (`format`) is accepted for API compatibility but ignored –
 * the input is always treated as a PEM string.
 */
export const asKey = async (pem: string, _format: string): Promise<Key> =>
  Key.fromPEM(pem);
